    ...
    static List<CtClass> toCtClasses(Collection<TransformInput> inputs, ClassPool classPool) {
        ...
        inputs.each {
            it.directoryInputs.each {
                ...
                System.out.println("it.file.absolutePath is   " + it.file.absolutePath);
                classPool.insertClassPath(it.file.absolutePath)
                org.apache.commons.io.FileUtils.listFiles(it.file, null, true).each {
                    System.out.println("it.absolutePath is   " + it.absolutePath);
            ...
            it.jarInputs.each {
                classPool.insertClassPath(it.file.absolutePath)
                System.out.println("jar it.file.absolutePath is   " + it.file.absolutePath);
                def jarFile = new JarFile(it.file)
                Enumeration<JarEntry> classes = jarFile.entries();
                while (classes.hasMoreElements()) {
                    JarEntry libClass = classes.nextElement();
                    String className = libClass.getName();
                    System.out.println("jar className is   " + className);
